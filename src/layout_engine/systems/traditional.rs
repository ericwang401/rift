...
impl LayoutSystem for TraditionalLayoutSystem {
    ...
    fn windows_share_a_stack(&self, layout: LayoutId, a: WindowId, b: WindowId) -> bool {
        let (Some(node_a), Some(node_b)) = (
            self.tree.data.window.node_for(layout, a),
            self.tree.data.window.node_for(layout, b),
        ) else {
            return false;
        };
        if node_a == node_b {
        ...
        let map = self.map();
        // Siblings, not merely both somewhere under the same stack: a stack
        // hands each child its whole rect, so two windows one level apart —
        // a window and a member of a split beside it — do not share a place.
        match (node_a.parent(map), node_b.parent(map)) {
            (Some(parent_a), Some(parent_b)) => {
                parent_a == parent_b && self.layout(parent_a).is_stacked()
            }
            _ => false,
        }
    }
